                ...
                res = self.socket.recv_from(&mut rx_buf) => {
                    ...
                    let (header, remainder) = match hubpack::deserialize::<Header>(&msg_buf) {
                        Err(e) => {
                            ...
                            // TODO-completeness: We may have an outstanding
                            // request that the peer meant this message to be
                            // a response to. Unfortunately, we don't really
                            // know if that's the case because we can't parse
                            // the message at all. We need to decide how to
                            // handle this, such as taking a limited number of
                            // these before failing an outstanding request, on
                            // the assumption that the peer means their response
                            // to apply to it.
                            todo!("Handle header deserialization failures");
                        }
                        Ok(m) => m,
                    };
                    ...
                    // TODO-cleanup: Decide which of this to keep, if any.
                    /*
                    // Deserialize the message itself.
                    let (message, remainder): (Message, _) = match hubpack::deserialize(&rx_buf) {
                        Err(e) => {
                            ...
                            let Ok((header, _)) = hubpack::deserialize::<Header>(&rx_buf) else {
                                error!(
                                    self.log,
                                    "failed to deserialize message header";
                                    "peer" => peer,
                                ...
                                // Take any outstanding response and inject an
                                // error on its response channel.
                                if let Some(request) = self.outstanding_request.take() {
                                    request.response_tx.send(
                                        Err(Error::Protocol(ProtocolError::Serialization))
                                    ).unwrap();
                                }
                                continue;
                            };
                            ...
                            // is useless.
                            //
                            // How we interpret the error depends on the outer
                            // message version that we get. If it _should_ be
                            // compatible with ours (it is between `MIN` and
                            // `CURRENT`), then we assume this is really a
                            // deserialization failure due to a corrupt buffer
                            // or similar. If the version is _ahead_ of ours, we
                            // assume they sent us a message we can't interpret,
                            // i.e., there's a version mismatch.
                            let err = match message.header.version {
                                message::vesion::outer::MIN..=message::version::outer::CURRENT => {
                                    ProtocolError::Serialization
                                }
                                _ => {
                                    ProtocolError::VersionMismatch {
                                        expected: message::version::outer::CURRENT,
                                        actual: message.header.version,
                                    }
                                }
                            };
                            if let Some(request) = self.outstanding_request.take() {
                                request.response_tx.send(Err(Error::Protocol(err)).unwrap();
                            }
                            continue;
                        }
